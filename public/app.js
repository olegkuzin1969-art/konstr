...
<h2>6. Оплата</h2>
<p>Оплата услуг в Сервисе осуществляется внутренней валютой BYE с личного баланса Пользователя. Пополнение баланса BYE производится способами, указанными на Сайте (банковские карты, платёжные системы и др.). Факт пополнения подтверждается электронным чеком платёжного провайдера.</p>
...
function getLetterPreviewFromData(f) {
  ...
  // В черновике (предпросмотр на сайте) показываем только шапку и заголовок,
  // чтобы основной юридический текст был доступен только в оплаченной версии (PDF / финальный документ).
  const parts = [];
...
function applyLanguageToShell() {
  ...
  if (navLinks.length >= 6) {
    ...
    navLinks[3].textContent = dict.nav.faq;
