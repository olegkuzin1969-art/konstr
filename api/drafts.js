...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (!draft || typeof draft !== 'object') return res.status(400).json({ error: 'Некорректные данные черновика' });
      const row = { user_id: userId, data: draft, updated_at: new Date().toISOString() };
      const { data: inserted, error } = await supabase.from('drafts').insert(row).select().single();
      if (error) return res.status(500).json({ error: error.message });
      return res.status(200).json({ draft: inserted });
    }

    if (req.method === 'PUT') {
      const { id, data: draft } = body;
      if (!id || !draft || typeof draft !== 'object') return res.status(400).json({ error: 'Некорректные данные' });
      const { data: updated, error } = await supabase
        ...
        .single();
      if (error) return res.status(500).json({ error: error.message });
      if (!updated) return res.status(404).json({ error: 'Черновик не найден' });
